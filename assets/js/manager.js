...
document.getElementById('startup').addEventListener('input', function (evt) {
  file.get(currentUser, 'settings', {}).then(function (result) {
    ...
    file.saveGlobal('settings', result).then(function () {
      console.log(`Successfully saved startup: ${document.getElementById('startup').checked}`)
        ...
        if (result2.length > 0) {
          ...
          result['Evaluations'].forEach(function (element) {
            file.get(currentUser, "viewedGrades").then(function (result3) {
              ...
              if (!viewed) {
                console.log(`Új jegy: ${element["Subject"]} - ${element["Value"]}`)
...
document.getElementById('notifications').addEventListener('input', function (evt) {
  file.get(currentUser, 'settings', {}).then(function (result) {
    result['notifications'] = document.getElementById('notifications').checked
    file.saveGlobal('settings', result).then(function () {
      if (notifications) {
        startNotificationListener();
      }
