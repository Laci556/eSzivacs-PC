...
const loggerNotification = LoggerUtil(
  '%c[Értesítések]',
  'color: #ffaaff; font-weight: bold'
);
          ...
          kreta
            ...
            .then(function(result, err) {
              ...
              result.forEach(function(element) {
                van = true;
                todayClasses += `<li class="collection-item"><div class="truncate">${
                  element['Subject']
                } <a href="#" class="secondary-content">${new Date(
                  element['StartTime']
                ).getHours()}:${new Date(
                  element['StartTime']
                ).getMinutes()} - ${new Date(
...
// JEGYEK TÁBLÁZAT FORMÁBAN
function renderGradesTable(row) {
  var nameOfType = 'Tablazat';
  ...
  // TODO: táblázat létrehozása

  let table = document.createElement('table'); // Táblázat létrehozása
  let tableHeader = document.createElement('thead');
  let headerRow = document.createElement('tr');
  let tantargyak = document.createElement('td');
  tantargyak.innerHTML = 'Tantárgyak';
  headerRow.appendChild(tantargyak); // Header a hónapok számával //? vagy inkább a nevükkel kellene?
  for (let i = 8; i < 18; i++) {
    headerRow.appendChild(tableCell((i % 12) + 1));
  }
  let average = document.createElement('td');
  average.innerHTML = 'Átlag';
  headerRow.appendChild(average);
  tableHeader.appendChild(headerRow);
  table.appendChild(tableHeader);

  jegyek.MidYear.forEach(element => {
    let subjectRow = document.createElement('tr'); // Tantárgyanként új sor
    let subjName = tableCell(element.name);
    subjectRow.appendChild(subjName);
    ...
    for (let i = 0; i < 10; i++) {
      ...
      for (let j = 0; j < grades.length; j++) {
        // if (grades[i]) {
        if (new Date(grades[j].Date).getMonth() == h) {
          let color =
            grades[j].Weight == '50%'
              ? 'blue'
              : grades[j].Weight == '100%'
              ...
              ? 'red'
              : 'yellow';
          gradesInMonth += ` <span style='color: ${color}'>${
            grades[j].NumberValue
          }</span>`;
        }
        //}
      }
      gradesInMonth = gradesInMonth.substring(1);
      subjectRow.appendChild(tableCell(gradesInMonth));
    }
    subjectRow.appendChild(
      tableCell('<span style="color: green">' + element.avg + '</span>')
    );
                                                ...
                                                ora['Teacher']
														...
														<div class="collapsible-body">
																<table>
																		...
																		<tr>
																				<td><b>Időtartam</b></td>
																				<td>${ora['StartTime'].split('T')[1].split(':')[0]}:${
      ...
      loadUserDatas().then(function(result) {
        file.get(currentUser, 'viewedGrades', []).then(function(result2) {
          if (result2.length > 0) {
            ...
            loggerNotification.log(
              'Még nem kaptál értesítést a jegyeidről. A mostantól kapott jegyeidről már fogsz!'
            );
            var grades = [];
            result['Evaluations'].forEach(function(element) {
              var jegy = {
...
if (!isDev) {
  ...
  loggerAutoUpdater.log(
    'Fejlesztési környezetben az Automatikus frissítési szolgáltatás le van tiltva.'
  );
}
    ...
    switch (arg) {
      ...
      case 'update-available':
        ...
        if (process.platform === 'darwin') {
          info.darwindownload = `https://github.com/pepyta/eSzivacs-PC/releases/download/v${
            info.version
          }/eSzivacs.dmg`;
          showUpdateUI(info);
        }
      ...
      case 'update-downloaded':
        loggerAutoUpdaterSuccess.log(
          'A v' + info.version + ' frissítés készen áll a telepítésre.'
        );
        changeUpdateState('build', 'Az új frissítés készen áll a telepítésre!');
        var toastHTML =
          '<span>Új frissítés érhető el!</span><button id="quitAndInstall" class="btn-flat toast-action">Újraindítás</button>';

        M.toast({ html: toastHTML });
        document
          .getElementById('quitAndInstall')
      ...
      case 'update-not-available':
        ...
        changeUpdateState(
          'done',
          'Az eSzivacs legfrisseb verzióját használod!'
        );
        break;
      case 'ready':
...
document.getElementById(
  'version'
).innerHTML = require('./../package.json').version;
